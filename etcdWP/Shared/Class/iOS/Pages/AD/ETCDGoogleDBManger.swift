...
import Foundation
import WCDBSwift
class ETCDGoogleDBManger: NSObject {
    
    static let share = ETCDGoogleDBManger()

    let dataBasePath = URL(fileURLWithPath:NSSearchPathForDirectoriesInDomains(.documentDirectory,
                                                                               .userDomainMask,
                                                                               true).last! + "/etcddb/etcdgood.db")
...
extension ETCDGoogleDBManger {
    ...
    //查询
    func getValue<T: TableDecodable>(
        on propertyConvertibleList: [PropertyConvertible],
        ...
        do {
            ...
            debugPrint("getValue error \(error.localizedDescription)")
        }
        return list
    }
}
