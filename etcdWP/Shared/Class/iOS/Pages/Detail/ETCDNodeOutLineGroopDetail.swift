//
//  ETCDNodeOutLineGroopDetail.swift
//  etcdWP (iOS)
//
//  Created by Google on 2022/8/9.
//

import SwiftUI
struct ETCDNodeOutLineGroopDetail: View {
                ...
                content: {
                    if isExpanded {
                        ForEach(node[keyPath: childKeyPath]!) { childNode in
                            ETCDNodeOutLineGroopDetail(callback:callback, node: childNode, childKeyPath:childKeyPath,isExpanded:isExpanded)
                        }
                    }
                },
                label: {
